...
async function previewDetails(url){
  ...
  if(vinted){
    ...
    const exactPrice=cleanListingPrice(d.vintedPrice)||cleanListingPrice(d.vintedTotal)||structured.price||cleanListingPrice(d.vintedDescription)||cleanListingPrice(text)||det.price||'';
...
$('#savedBrowseBtn').onclick=()=>{savedMode='browse';renderBoard()};$('#savedCompareBtn').onclick=()=>{savedMode='compare';renderBoard();setTimeout(()=>refreshSavedDetails(false),100)};$('#refreshDetails').onclick=()=>refreshSavedDetails(true);:currency==='EUR'?'€':currency==='GBP'?'£':currency+' ';
